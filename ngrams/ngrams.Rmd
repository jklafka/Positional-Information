...
```{r childes}
utterances <- get_utterances(corpus = "Plunkett") %>% 
  filter(speaker_role != "Target_Child") %>%
  rename(words = gloss)

# unigrams to check against previous results
unigrams <- map(2:10, ~ngram_entropy(utterances, .x, 1)) %>% 
  bind_rows()

# bigrams
bigrams <- map(3:10, ~ngram_entropy(utterances, .x, 2)) %>% 
  bind_rows()

# trigrams
trigrams <- map(4:10, ~ngram_entropy(utterances, .x, 3)) %>% 
  bind_rows()

# quadgrams
quadgrams <- map(5:10, ~ngram_entropy(utterances, .x, 4)) %>%
  bind_rows()

unigrams %>%
  bind_rows(bigrams, trigrams, quadgrams) %>%
  mutate(Language = "Danish") %>%
  write_feather("CHILDES/danish_adult.feather")
```
