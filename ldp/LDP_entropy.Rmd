...
get_ses_utterances <- function(status, utterances, home_visits) {
  income <- home_visits %>% filter(income_category %in% status) 
  
  ids <- unique(income$id) %>% unlist()
  
  utterances %>% filter(subject %in% ids)
}

ses_utts <- get_ses_utterances(c(6), utterances, home_visits)
```
...
entropies <- map(2:10, ~get_unigram_entropies("child", .x, ses_utts)) %>%
  bind_rows()
