...
When people communicate in speaking or writing, they can have a variety of different goals. Perhaps the most important of these goals is to communicate information. If we take this as the primary goal of communication, then how information is transmitted should have regular properties. There is a rich literature on information, dating back to Claude Shannon’s theory of information (1948). We know from Shannon’s work that rate of information transmission should be constant and as close as possible to the capacity of the communication channel. Levy & Jaeger (2007) introduce the *uniform information density* hypothesis, arguing that speakers structure utterances so as to optimize information density. Viewing human communication as a noisy channel, the authors argue that speakers and writers should attempt to communicate information as evenly as possible and as close to the channel capacity as possible. Their evidence comes in the form of the use or lack thereof of “that”-complementizers in introducing English relative clauses. When the word preceding the relative clause or the initial word of the relative clause has high information content, then the authors argue that speakers are more likely to use the complementizer. The authors use adult-adult conversations from the Penn Treebank corpus (CITATION). 

Zhan and Levy (2018) study Mandarin Chinese classifier use, and find that the use of specific classifiers, versus general classifiers, appears more often in cases where the production of the corresponding noun is more difficult than when the production is easier, as would be predicted by the UID perspective. 

Let us assume that the UID perspective is correct. Then entropy should monotonically and uniformly increase at all word positions across utterances on average. We will find a contradiction with this claim through examining the average entropy at each position of a sentence for both speech and text data. 

In Yu et al. (2016), the authors challenge the UID perspective through computing the unigram positional entropy within the text portion of the British National Corpus (BNC). We follow Yu et al. (2016) in using the following formula for each word position X of sentences of fixed length k from the corpus, where each $w$ is a word occurring in position $X$ and $p(w)$ is the number of times word $w$ occurs in position X divided by the number of total words that occur in position $X$ i.e. the number of sentences of length $k$. [Give the equation its own paragraph]

$$H(X) = \sum\limits_w p(w)\log\big(p(w)\big)$$

The result of this method can be plotted for each utterance length as an *entropy curve*, which can be visually compared across utterance length to observe the how the unigram entropy changes across absolute positions in each of the utterances


# Methods

For spoken data, we choose to examine child-adult conversations in the CHILDES TalkBank (Brown 1973; MacWhinney 2000) corpora database of spoken adult-child conversations in multiple languages. We expect based on the UID account that the entropy curves of the children will be significantly lower than those of the adults, due to the children's comparatively lower channel capacity in information transmission [more support]. We first used the Brown and Providence English corpora from CHILDES. The Brown corpus contains individual recordings of conversations between three children between 1.5 and 6-years-old and their families in their homes. The Providence corpus recorded interactions between children between 1 and 3 years old and their parents in the home. We divided each corpus by speaker into child and non-child categories. We further divided the corpora by utterance length, so that all sentences of length $k$ (e.g. $6$) were grouped together. Finally, within each utterance length, we computed the unigram entropy measure for each position.  
