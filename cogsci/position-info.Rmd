...
Our plots for three representative utterance lengths are below. 

```{r read_and_plot_switchboard}
switchboard_entropies <- read_csv(here("data/switchboard_entropies.csv")) %>%
  filter(length %in% c(5, 7, 9))

ggplot(switchboard_entropies, aes(x = word_order, y = empirical_entropy,
                      ymin = ci_lower, ymax = ci_upper)) +
  facet_wrap(~ length) + 
