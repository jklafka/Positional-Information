...
get_noneng_entropies <- function(role, sen_length, utterances) {
...
entropies <- map(2:10, ~get_noneng_entropies("not child", .x, okayama_utterances)) %>%
...
entropies_child <- map(2:10, ~ get_noneng_entropies("Target_Child", .x, okayama_utterances)) %>%
  bind_rows() %>% filter(word_order <= length)
...
ggplot(entropies_all, aes(x = word_order, y = empirical_entropy,
  ...
  geom_pointrange(position = position_dodge(.25)) +
  geom_smooth(se = F) +
  ggtitle("Child and Adult Positional Entropy vs. Word Position (Japanese Okayama corpus)")
```