...
entropies <- map(2:10, ~get_entropies("not child", .x, shiro_utterances)) %>%
  bind_rows()
...
entropies_child <- map(2:10, ~ get_entropies("Target_Child", .x, shiro_utterances)) %>%
  bind_rows()
...
ggplot(entropies_all, aes(x = word_order, y = empirical_entropy,
  ...
  geom_pointrange(position = position_dodge(.25)) +
  geom_smooth(se = F) +
  ggtitle("Child and Adult Positional Entropy vs. Word Position (Spanish Shiro corpus)")
```