...
get_noneng_entropies <- function(role, sen_length, utterances, langue) {
  
  if(role == "Target_Child") {
    sub_utterances <- utterances %>%
      filter(speaker_role == "Target_Child") %>%
      mutate(stems = wordStem(gloss, language = langue)) %>% 
...
ggplot(entropies_all, aes(x = word_order, y = empirical_entropy,
  ...
  geom_pointrange(position = position_dodge(.25)) +
  geom_smooth(se = F) +
  ggtitle("Child and Adult Positional Entropy vs. Word Position (German Wagner corpus with stemming)")
```