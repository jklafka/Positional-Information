...
```{r, echo=F, fig.height=2, fig.cap="Switchboard frequency-based information curves"}
switch_results %>% 
  filter(context == 0) %>%
...
```{r, echo=F,fig.height=2,fig.cap="Switchboard bigram context-based information curves"}
switch_results %>% 
  filter(context == 1) %>%
  ggplot(aes(x = word_order, y = empirical_stat, ymin = ci_lower, ymax = ci_upper)) + 
    geom_point() + 
...
```{r, echo=F,fig.height=2,fig.cap="Switchboard trigram context-based information curves"}
switch_results %>% 
  filter(context == 2) %>%
  ggplot(aes(x = word_order, y = empirical_stat, ymin = ci_lower, ymax = ci_upper)) + 
    geom_point() + 
...
```{r, echo=F, fig.cap="Some linear model results from Wikipedia and WALS features"}
sf <- wiki_lr %>% 
