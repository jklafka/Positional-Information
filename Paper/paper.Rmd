...
```{r, echo=F, fig.height =2, fig.cap="Providence context-based information curves"}
childes_results %>% 
  filter(Language == "English", context %in% c(1, 2), length %in% c(5, 7, 9)) %>%
  ggplot(aes(x = word_order, y = s, color = speaker_role)) + 
    geom_point() + 
    facet_wrap(context~length) + 
    scale_x_continuous(breaks = 1:9) + 
    geom_line() + 
    xlab("Word position") + 
    ylab("Mean information")
...
```{r wals features vs cosine similarity}
wiki_results %>% 
...
```{r ldn features vs cosine similarity}
wiki_results %>% 
