...
childes_results <- read_feather(here("../surprisal/Data/childes_words.feather"))
wiki_results <- read_feather(here("../surprisal/Data/all_spaces.feather"))
switchboard <- read_feather(here("../surprisal/Data/switchboard_words.feather"))
...
```{r, echo=F, fig.height =2, fig.cap="Providence frequency-based information curves"}
childes_results %>% 
  filter(Language == "English", context == 0, length %in% c(5, 7, 9)) %>%
  ggplot(aes(x = word_order, y = s, color = speaker_role)) + 
    geom_point() + 
    facet_wrap(~length) + 
...
```{r, echo=F, fig.height =2, fig.cap="Providence bigram context-based information curves"}
childes_results %>% 
  filter(Language == "English", context == 1, length %in% c(5, 7, 9)) %>%
  ggplot(aes(x = word_order, y = s, color = speaker_role)) + 
    geom_point() + 
    facet_wrap(~length) + 
    scale_x_continuous(breaks = 1:9) + 
...
```{r, echo=F, fig.height =2, fig.cap="Providence trigram context-based information curves"}
childes_results %>% 
  filter(Language == "English", context == 2, length %in% c(5, 7, 9)) %>%
  ggplot(aes(x = word_order, y = s, color = speaker_role)) + 
    geom_point() + 
    facet_wrap(~length) + 
    scale_x_continuous(breaks = 1:9) + 
...
```{r, echo=F, fig.cap="Some trigram information curves from the Wikipedia data"}
wiki_curves %>%
  filter(gram == "Trigram") %>% 
...
```{r, echo = F, fig.cap="ldn features vs cosine similarity"}
wiki_results %>% 
