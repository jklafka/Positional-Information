...
How these pressures shape language evolution??
...
In our contextual models, we face the problem of overfitting: we only train our model on those utterances which occur in the corpus and test our model on the same utterances. This ignores possible other utterances which the speakers could have produced, e.g. the words "I", "saw" and "bears" are in the corpus vocabulary, which the speaker may not have produced in the corpus but could have produced. To combat this issue, we use modified Kneser-Ney smoothing [@chen1999empirical]. 

In this smoothing technique, we filter the ngram counts from the corpus to only those above a threshold, ignoring rare ngrams. We then apply *absolute discounting* to all of the ngram, which reduces the counts of all observed ngrams in the corpus by a fixed number, reducing the impact of uncommon ngrams on our surprisal calculations. We *interpolate* lower order ngram counts when computing bigrams and trigrams: this means computing a weighted average of the ngram counts along with lower order ngram counts, in case the higher order counts for a particular ngram are rare. The main insight of Kneser-Ney smoothing is to weight ngram counts by how often the final word occurs as a continuation of any previous words in the corpus, downweighting common unigrams which occur only in a few contexts. This version of modified Kneser-Ney smoothing is in the Kenlm language modeling toolkit [@heafield2013scalable]. 
