...
extract_gloss_tokens <- function(role, min_length, utterances) {
  ...
  tokens <- lengths %>% 
    ...
    mutate(utterance_id = 1:n()) %>%
    unnest_tokens(word, gloss) %>%
>>>>>>> ff9c46721b567b45eeaa7fe7e80bdb7e59ae5467
    group_by(utterance_id) %>%
    mutate(word_order = 1:n()) %>%
    group_by(length, word_order, word) %>%
    filter(word_order <= length) %>% 
...
=======
get_slopes <- function(role, min_length, utterances, stemgloss) {
  
  if (stemgloss == "stem") {
    tokens <- extract_stem_tokens(role, min_length, utterances)  
  } else {
    tokens <- extract_gloss_tokens(role, min_length, utterances)
  }
...
```{r slope_analysis}
#get_slopes("not child", 6, py$xmf_df, "gloss")
gtx <- extract_gloss_tokens("not child", 6, py$xmf_df)
start_slopes <- replicate(1000, start_slope(gtx)) %>% 
    as_data_frame() %>%
    summarise(mean = mean(value), ci_upper = quantile(value, .975), 
              ci_lower = quantile(value, .025))
print(start_slopes)
...
```{r analyze_entropies, include=FALSE}
>>>>>>> ff9c46721b567b45eeaa7fe7e80bdb7e59ae5467
