...
fair_wf <- wf[rowSums(!is.na(wf)) > 50,] %>%
  filter(language %in% LANGUAGES) %>%
  select(-wals.code, -latitude, -longitude, -glottocode) %>%
  as_tibble() %>%
  clean_names()
...
mean_impute <- function(col) {
  counts <- col %>%
    ...
    table() %>%
    sort(decreasing = T) 
  col[is.na(col),] <- labels(counts[1])
  return(col)
}

for (i in 1:ncol(mean_wf)) {
  mean_wf[, i] <- mean_impute(mean_wf[, i])
}

# Phonology `1A`:`19A`
# Morphology `20A`:`29A`
# Nominal Categories `30A`:`57A`
# Nominal Syntax `58A`:64A`
# Verbal syntax `65A`:`80A`
# Word order `81A`:`97A`, `143A`, `144A`
# Simple clauses `98A`:`120A`
# Complex sentences `121A`:`128A`
# Lexicon `129A`:`138A`
# Sign languages `139A`:`140A`

wa <- fair_wf %>% 
  select(x1a:x10a, language) %>%
  group_by(language) %>%
  slice(1) 

wa_langs <- wa %>%
  gather(measure, value, -language) %>%
  group_by(language) %>%
  summarise(value = mean(!is.na(value))) %>%
  arrange(desc(value)) %>%
  filter(value > .6) %>% 
  pull(language)
...
wa_sub <- wa %>%
  filter(language %in% wa_langs) %>%
  column_to_rownames("language") %>%
  ungroup()
 # mutate_each(~as.numeric(as.factor(.x)))

wa_out <- wa_sub %>%
  MIMCA(ncp = 1)

wa_imputed <- wa_out[["res.MI"]] %>%
  map(~mutate(.x, language = wa_langs)) %>%
  bind_rows(.id = "boot") %>%
  group_by(language, boot) %>%
  gather(measure, value, -language, -boot) %>%
...
models %>%
  ...
  write_feather(here("word_order_glm_absolute.feather"))
```
