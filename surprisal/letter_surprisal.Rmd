...
```{r set up corpus}
prov_utterances <- get_utterances(corpus = "Palasis")

french_utterances <- prov_utterances %>%
...
phone_counts <- adult_phonemes %>% 
  group_by(utterance_id, word_id) %>% 
  summarise(length = max(word_length)) %>% 
  ungroup() %>% 
  count(length)
```
...
```{r plot results}
letter_unigrams <- letter_unigrams %>% mutate(gram = "Unigram") 
letter_bigrams <- letter_bigrams %>% mutate(gram = "Bigram") 
letter_trigrams <- letter_trigrams %>% mutate(gram = "Trigram") 
letter_unigrams %>% bind_rows(letter_bigrams, letter_trigrams) %>% write_feather(here("Data/palasis_letters.feather"))
