...
```{r}
four_lengths <- adult_phonemes %>%
  filter(word_length == 3) %>%
  group_by(utterance_id, word_id, phone_order) 

four_words <- four_lengths %>%
  ungroup() %>%
  distinct(word, phone_order, phone) %>%
  group_by(word) %>%
  nest()
...
word1 %>%
  ...
  left_join(four_lengths %>%
  spread(phone_order, phone)) %>%
  distinct(utterance_id, word_id, word) %>%
  group_by(word) %>%
  summarise(n = n())
