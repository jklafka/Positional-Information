...
child_surprisals %>%  
  ...
  ggplot(aes(x = word_order, y = mean)) +
    ...
    ggtitle("Unigram POS surprisals") + 
    xlab("Word position") +
    ylab("Mean surprisal") +
...
```{r pos bigrams}
child_bigrams_prep <- child_utterances %>% 
  group_by(utterance_id) %>%
...
child_trigrams <- child_utterances %>%
  ...
  ungroup() %>%
  left_join(child_bigrams_prep, by = c("lag_word2" = "lag_word", "lag_word1" = "word")) %>%
...
trigram_surprisals_prep <- child_utterances %>%
  ...
  left_join(select(child_bigrams_prep, lag_word, word, cond_p), by = c("lag_word1" = "lag_word", "word" = "word")) %>% 
  left_join(child_unigrams)
...
trigram_surprisals %>%  
  ...
  ggplot(aes(x = word_order, y = mean_s)) +
    facet_wrap(~length, scales = "free") +
    ggtitle("Trigram POS surprisals") + 
    xlab("Word position") +
    ylab("Mean surprisal") +
    # scale_x_continuous(breaks = 1:7) + 
    geom_line(size = 1) +
