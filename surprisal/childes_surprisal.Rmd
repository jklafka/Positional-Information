...
surprise <- bind_rows(child_surprisals %>%
  mutate(speaker_role = "Child") %>%
  bind_rows(mutate(adult_surprisals, speaker_role = "Adult")) %>%
...
adult_bigrams %>% 
  ...
  ggplot(aes(x = word_order, y = mean_s, color = speaker, label = speaker)) + 
  xlab("Word position") +
  ylab("Mean surprisal") +
  geom_point() + 
  geom_line() + 
...
label_data <- data_frame(length = 9, Language = "Spanish", word_order = 4,
                         speaker_role = c("Adult", "Child"), s = c(5,6.25))
...
label_data <- data_frame(length = 9, Language = "Spanish", word_order = 3,
                         speaker_role = c("Adult", "Child"), s = c(1.25,3.5))
...
all_bigrams <- all_bigrams %>% 
  bind_rows(mutate(bigram_surprisals_nonnull, speaker_role = "Child", Language = "Spanish"))
...
all_trigrams <- all_trigrams %>% 
  bind_rows(mutate(trigram_surprisals_nonnull, speaker_role = "Child", Language = "Spanish"))
