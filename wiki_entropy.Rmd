...
text = extract_texts(15)
am_df = get_sen_df(text)

am_df.to_csv("am_df.csv")
```

```{r slope_prep, include=FALSE, eval = F}
...
df <- read_csv("~/Documents/Labwork/Joe/Positional-Information/ar_df.csv")
...
ar_slopes <- get_wiki_slopes(min_length = 6, max_length = 50, utterances=df, 1000)
...
```{r slope_analysis, include = F, eval = F}
